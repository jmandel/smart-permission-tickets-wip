...
2.  **Layer 2: Ticket Validation (Permission Ticket Specific)**
    ...
    *   **Verify Identity Evidence:** If `subject_identity_evidence` or `requester_identity_evidence` is present, verify it per [Identity Evidence](#identity-evidence) — signature, evidence-issuer trust, temporal validity, client-identifier resolution, and demographic consistency with the party in that slot — plus the selected profile's assurance and claim parameters.
...
**Who obtained the evidence.** An embedded ID token records a sign-in that some application completed at the evidence issuer. The token's `iss` names the evidence issuer; its **client identifier** — the token's `aud` (and `azp`, when present), or another claim a ticket-type profile designates for this purpose — identifies the application the token was issued to. Two topologies are supported:

* **Issuer-obtained evidence.** The ticket issuer acted as a relying party at a third-party evidence issuer and ran the sign-in itself. The client identifier resolves to the ticket issuer.
* **Client-obtained evidence.** The client ran the sign-in and handed the resulting token to the ticket issuer during issuance. The client identifier resolves to the client.
...
**Resolving the client identifier.** The client identifier is checked at both ends of the ticket's life, and the check is the same at both: the verifier confirms that the evidence's client identifier refers to the same client it has authenticated. The identifiers may be literally identical, or known to refer to the same client through a process the verifier recognizes, such as a directory lookup or its own registration records.

* At issuance, before embedding client-obtained evidence, the issuer SHALL verify through a recognized mapping that the evidence's client identifier resolves to the same client it authenticated in the issuance ceremony. Absent a resolvable mapping, the issuer SHALL NOT embed the evidence. An issuer able to run its own sign-in MAY embed issuer-obtained evidence instead; an issuer that cannot, and whose selected ticket type requires the evidence, cannot mint the ticket.
* At redemption, the Data Holder SHALL resolve the evidence's client identifier to either the ticket issuer or the presenting client: identical to an identifier it bound to that party at registration, or known to refer to the same client through a process it recognizes. Profiles MAY allow only one of these. Resolving the identifier is what proves the sign-in happened as part of issuing or presenting this ticket rather than being harvested from some other application's sign-in.

The evidence's client identifier never names the Data Holder itself — Data Holders SHALL NOT expect their own identifier there. How a Data Holder learns which client identifiers belong to a ticket issuer is deployment configuration: issuer metadata, a trust-framework directory, or direct configuration.

Evidence identifying one client inside a ticket presented by a different party fails the presenting-client check by design: it prevents a sign-in completed at one application from laundering through another presenter. Ticket-type profiles that introduce delegation, where one party obtains a ticket and a different party presents it, SHALL address this case explicitly.

*Design note:* the client identifier may be self-asserted by the application when it configures itself at the evidence issuer, because it only benefits a presenter that can cryptographically authenticate as that client at the verifier. What counts as a recognized resolution is up to the trust framework or ticket-type profile: the identifiers may simply be identical (for example, a canonical client identifier carried as or alongside `aud`), or related through an out-of-band source such as a directory.
...
* Confirm who the evidence was issued to: resolve the token's client identifier to the ticket issuer or the presenting client, per the resolution rules above.
...
- If `subject_identity_evidence` or `requester_identity_evidence` is present, verify it per the base Identity Evidence rules (signature, evidence-issuer trust, temporal validity, client-identifier resolution, demographic consistency with the party in that slot) and any profile-defined assurance and claim requirements
...
- When embedding client-obtained identity evidence, verify through a recognized mapping that the evidence's client identifier resolves to the same client authenticated in the issuance ceremony; absent a resolvable mapping, omit the evidence (an issuer able to run its own sign-in MAY embed issuer-obtained evidence instead)
