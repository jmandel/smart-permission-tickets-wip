...
> **Open Question (OQ-3D): How does an issuer know which relationships are sensitive to reveal, and should suppression ever be enforced?** Suppressing a hint protects a care relationship only if the issuer knows the relationship needs protecting — that a site's name alone reveals what a withheld category covers, while a general hospital's does not. No directory carries that today. Suppression as written is advisory: a client holding a network-scoped ticket can probe by presenting it at a suppressed or guessed endpoint. The option space differs by issuer: a network issuer that operates the record locator knows the patient's sites and could pair suppression with a matching `data_holder_filter`, making the exclusion enforced rather than advisory — at a disclosure cost of its own, since a filter naming the patient's sites travels in the signed ticket to every Data Holder where it is presented, and each site learns the others. Data Holders whose patient population is itself the sensitive fact also have their own defenses: refusing redemption outright, or refusing tickets that withhold their specialty's categories. What should the spec say about each of these, and where would issuer-side knowledge of disclosure-sensitive sites come from?
