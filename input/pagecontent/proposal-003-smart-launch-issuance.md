...
> **Open Question (OQ-3D): How does an issuer know which relationships are sensitive to reveal?** Suppressing a hint protects a care relationship only if the issuer knows the relationship needs protecting — that a site's name alone reveals what a withheld category covers, while a general hospital's does not. No directory carries that today. Related: suppression is advisory, and a client holding a network-scoped ticket can probe by presenting it at a suppressed or guessed endpoint; Data Holders whose patient population is itself the sensitive fact have their own defenses (refusing redemption outright, or refusing tickets that withhold their specialty's categories), but the spec does not yet say anything about either side of this. What should it say, and where would issuer-side knowledge of disclosure-sensitive sites come from?
