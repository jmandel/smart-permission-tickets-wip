...
These use cases are not part of the near-term implementable catalog. Most are sketches: their `ticket_type` URIs are reserved, their field lists are unsettled, and each has unresolved questions. UC3 (Public Health Investigation) is the exception — fully modeled and deferred, not unfinished. They are kept here so the work is not lost, and to keep the [Use Case Catalog](use-case-catalog.html) focused on what implementers on the project are ready to build.
...
* **Access:** `permissions` required; `data_period` and `data_holder_filter` typical.
