...
Ticket types are identified by their `ticket_type` URIs and referred to by name. Public Health Investigation is fully modeled but has lived on [Future Use Cases](future-use-cases.html) since June 2026, alongside the other candidates; the near-term catalog holds the flows implementers on the project are ready to exercise.
