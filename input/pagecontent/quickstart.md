...
2. **Run your verification workflow** for the ticket type: identity proofing for the patient (UC1), both parties plus delegation authority (UC2), organizational and event context (UC3). What you must verify per type is in the [Use Case Catalog](use-case-catalog.html).
...
3. **Present the ticket** at the token endpoint: one POST with your client assertion and the ticket as `subject_token`, requesting SMART v2 scopes within what the ticket allows. ([The Request](index.html#transport-token-exchange-rfc-8693))
