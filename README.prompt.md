...
- **Build pipeline shape**: the local end-to-end build is `npm install` (root) → `npm --prefix scripts install` → `./_genonce.sh`. `_genonce.sh` runs `sync-spec-snippets` (regenerates JSON snippets in the prose from the canonical TypeScript schema), then `sushi .`, then `./_updatePublisher.sh` (fetches/refreshes the FHIR IG Publisher jar into `input-cache/`), then `java -jar input-cache/publisher.jar -ig .`.
